...
trait QuerySession extends Queryable {
  ...
  private def call[OP, T, R](
    ...
    receivers: Seq[T ⇒ R],
    outParmHandler: Option[(OutParms) ⇒ OP]): (OP, Seq[R]) = {
      def delegate(t: Iterator[T]): R = t.next
    try {
  ...
  protected def queryFromCall[T, R](token: Token[T], cs: CallStatement, parms: Seq[(String, Any)], receiver: T ⇒ R): R = {
    token.extractor match {
      ...
      case _ ⇒
        val (foo, bar) = call(token, cs, parms, Seq(receiver), None)
    }
  }
